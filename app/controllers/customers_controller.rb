class CustomersController < ApplicationController
  ...
  def show
    respond_to do |format|
      format.html
      format.json { render :json => @customer.to_json(:only => :email, :include => { :subscriptions => { :only => [ :expires_on ], :methods => :product_identifier } }) }
      format.xml { render :xml => @customer.to_xml(:only => :email, :include => { :subscriptions => { :only => [ :expires_on ], :methods => :product_identifier } }) }
    end
  end
